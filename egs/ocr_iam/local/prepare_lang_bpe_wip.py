...
def generate_lexicon(
    model_file: str, oov: str,
) -> Tuple[Lexicon, Dict[str, int], k2.SymbolTable]:
...
#    lexicon.append((oov, ["▁", sp.id_to_piece(sp.unk_id())]))
    ...
    word_sym_table.add("<eps>")
    lexicon = []
    exclude = ['<blk>', '<sos/eos>']
    for key, value in sorted(token2id.items()):
        if key in exclude:
            continue
        lexicon.append((key, [key]))
        word_sym_table.add(key)
    lexicon.append((oov, ["▁", sp.id_to_piece(sp.unk_id())]))
    return lexicon, token2id, word_sym_table
...
def main():
    ...
    lexicon, token_sym_table, word_sym_table = generate_lexicon(model_file, args.oov)
    ...
    write_mapping(lang_dir / "words.txt", word_sym_table._sym2id)
