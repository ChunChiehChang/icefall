...
class CnnTdnnLstm(nn.Module):
    def __init__(
        self, num_channels: int, num_features: int, num_classes: int, subsampling_factor: int = 3
    ) -> None:
        """
        Args:
          num_features:
        ...
        self.cnn = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.BatchNorm2d(num_features=1, affine=False),
            nn.Dropout(p=0.2),
        )
        ...
        self.lstms = nn.ModuleList(
            [nn.LSTM(input_size=256, hidden_size=256, num_layers=1) for _ in range(2)]
        )
        self.lstm_bnorms = nn.ModuleList(
            [nn.BatchNorm1d(num_features=256, affine=False) for _ in range(3)]
        )
        self.dropout = nn.Dropout(0.2)
        self.linear = nn.Linear(in_features=256, out_features=self.num_classes)
